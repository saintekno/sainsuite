...
class CI_Profiler
{
    ...
    protected function _compile_config()
    {
        ...
        foreach ($this->CI->config->config as $config => $val) {
            if ($val === true) {
                $output[$config] = $this->CI->lang->line('rp_profiler_true');
            } elseif ($val === false) {
                $output[$config] = $this->CI->lang->line('rp_profiler_false');
            } elseif (is_array($val)) {
    ...
    public function _compile_userdata()
    {
        ...
        foreach ($compiled_userdata as $key => $val) {
            if ($val === true) {
                $output[$key] = $this->CI->lang->line('rp_profiler_true');
            } elseif ($val === false) {
                $output[$key] = $this->CI->lang->line('rp_profiler_false');
            } elseif (is_array($val)) {
