...
class aauth_setup extends CI_model
{
    public function __construct()
    {
        ...
        $this->events->add_action('Do_settings_tables', array( $this, 'settings_tables' ));
        $this->events->add_action('Do_setup', array( $this, 'Do_setup' ));
    }
    ...
    **/
    ...
    public function Do_setup()
    {
        ...
        if ($this->users->auth->has_member('master')) {
            redirect(array( 'login?notice=access-denied' ));
        }
    }
    ...
    **/
    ...
    public function final_config()
    {
        ...
        if ($create_user != 'user-created') {
            $this->events->add_filter('validating_Do_setup', array( $this, 'preparing_errors' ));
        }
    }
    ...
    public function preparing_errors()
    {
        if (is_array($this->users->auth->errors)) {
            foreach ($this->users->auth->errors as $error) {
                $this->notice->push_notice(Do_error($error));
            }
        }
    }
