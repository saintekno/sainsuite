...
class Aauth
{
    ...
    public function __construct()
    {
        ...
        $this->CI->load->module_helper('recaptchalib');
        // $this->CI->lang->load('aauth');


        // config/aauth.php
        $this->CI->load->addin_config('aauth', 'aauth');
    ...
    public function create_user($email, $pass, $name)
    {
        ...
        if ($this->user_exsist_by_email($email)) {
            $this->CI->notice->push_notice( Do_error( $this->CI->lang->line('aauth_error_email_exists') ) );
            $valid = false;
        }
        if ($this->user_exsist_by_name($name)) {
            $this->CI->notice->push_notice( Do_error( $this->CI->lang->line('aauth_error_username_exists') ) );
            $valid = false;
        }

        if (! valid_email($email)) {
            $this->CI->notice->push_notice( Do_error( $this->CI->lang->line('aauth_error_email_invalid') ) );
            $valid = false;
        }
        if (strlen($pass) < 5 or strlen($pass) > $this->config_vars['max']) {
            $this->CI->notice->push_notice( Do_error( $this->CI->lang->line('aauth_error_password_invalid') ) );
            $valid = false;
        }
        if ($name !='' && !ctype_alnum(str_replace($this->config_vars['valid_chars'], '', $name))) {
            $this->CI->notice->push_notice( Do_error( $this->CI->lang->line('aauth_error_username_invalid') ) );
            $valid = false;
        }
        if (empty($name)) {
            $this->CI->notice->push_notice( Do_error( $this->CI->lang->line('aauth_error_username_required') ) );
            $valid = false;
        }
    ...
    **/
     ...
     */
    public function list_users($group_par = false, $offset = false, $limit = false, $include_banneds = false)
    { // Called
    ...
    **/
    ...
    public function is_public_group($group_name)
    {
