...
class Do_setup extends Eracik_Controller
{
    ...
    public function index()
    {
        ...
        // $this->load->model( 'Do_setup' );
        $this->load->view('do-setup/header');
    ...
    public function site()
    {
        ...
        $this->events->do_action('Do_setup');
        ...
        if ($this->form_validation->run()) {
            ...
            if ($exec == 'eracik-installed') {
                redirect(array( 'login?redirect=dashboard/index&notice=' . $exec . ( @$_GET[ 'lang' ] ? '&lang=' . $_GET[ 'lang' ] : '') ));
            }
        ...
        // Outputing
        Html::set_title(sprintf(__('Site & Master account &mdash; %s'), get('app_name')));
        $this->load->view('do-setup/header');
