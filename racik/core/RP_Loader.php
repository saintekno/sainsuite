 ...
 * Racik Loader
 *
 * @package Racik\Core\RP_Loader
 */
class RP_Loader extends MX_Loader
{
    /**
     * Constructor
     *
    ...
    public function __construct()
    {
        ...
        $this->_ci_library_paths = array(APPPATH, RPPATH, BASEPATH);
        $this->_ci_helper_paths  = array(APPPATH, RPPATH, BASEPATH);
        $this->_ci_model_paths   = array(APPPATH, RPPATH);
        $this->_ci_view_paths    = array(
            APPPATH . 'views/' => true,
            RPPATH . 'views/'  => true
        );

        log_message('debug', 'RP_Loader Class Initialized');
     ...
     *
     * Overridden to correct path order and load RP_ libraries.
     *
    ...
    protected function _ci_load_stock_library($library_name, $file_path, $params, $object_name)
    {
        ...
        foreach (array("RP_{$library_name}", $myLibraryName) as $subclass) {
            foreach ($paths as $path) {
                if (file_exists($path = "{$path}libraries/{$file_path}{$subclass}.php")) {
                    include_once($path);
                    if (class_exists($subclass, false)) {
                        $prefix = $subclass == $myLibraryName ? config_item('subclass_prefix') : 'RP_';
                        break;
                    } else {
