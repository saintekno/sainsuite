 ...
 *
 * @package Racik\Libraries\Assets
 */
class Assets
{
     ...
     *
     * Compared to lang('rp_language_direction') to indicate direction and
     * appended to the file name to check for existence of the file when
     * lang('rp_language_direction') == $rtl_postfix
     */
    ...
    public static function init()
    {
        ...
        if (($assetsDirs === false || $assetsDirs === null)) {
        ...
        if (self::$ci->config->item('assets.directories')) {
            ...
            self::$directories['base']  = trim(self::$ci->config->item('assets.directories')['base'] ?: 'assets', '/');
            self::$directories['cache'] = trim(self::$ci->config->item('assets.directories')['cache'] ?: 'cache', '/');

            // Set to default because this directory was not in the previous
            // config locations.
            self::$directories['module'] = 'module';

            $assetFolders = self::$ci->config->item('assets.directories')['base'] ?: array('js' => 'js', 'css' => 'css', 'image' => 'images');
    ...
    private static function find_files($files = array(), $type = 'css', $bypass_inheritance = false)
    {
        ...
        $rtl_set = lang('rp_language_direction') == $rtl;
            ...
            if (empty($module)) 
            {
                ...
                foreach ($paths as $path) 
                {
                    if (self::$debug) 
                    {
                        ...
                        echo "<li>{$site_path}" . self::$directories['base'] . "/{$clean_type}/{$file}{$type}</li>
                            </ul>" . PHP_EOL;
                    }
    ...
    private static function get_file_array($sitePath = '', $path = '', $file = '', $type = '', $media = '')
    {
        ...
        if (lang('rp_language_direction') == self::$rtl_postfix) 
        {
