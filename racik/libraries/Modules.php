 ...
 *
 * @package racik\Libraries\Modules
 */
class Modules
{
    ...
    public static function autoload($class)
    {
        ...
        // Autoload racik core classes.
        if (strstr($class, 'RP_')
            && is_file($location = RPPATH . "core/{$class}.php")
        ) {
            include_once($location);
            return;
        }
        ...
        // Autoload racik library classes.
        if (is_file($location = RPPATH . "libraries/{$class}.php")) {
            include_once $location;
            return;
        }
    }
    ...
    public static function files($module_name = null, $module_folder = null, $exclude_core = false)
    {
        ...
        foreach (Modules::folders() as $path) {
            ...
            if ($exclude_core
                && stripos($path, 'racik/modules') !== false
            ) {
                continue;
            }
     ...
     *
     * @param bool $exclude_core Whether to exclude the racik core modules.
    ...
    public static function list_modules($exclude_core = false)
    {
        ...
        foreach (Modules::folders() as $folder) {
            // If excluding core modules, skip the core module folder.
            if ($exclude_core && stripos($folder, 'racik/modules') !== false) {
                continue;
            }
