...
<div class="well shallow-well">
	<span class="filter-link-list">
        ...
        if ($filter_type == 'first_letter') {
			echo anchor($index_url, lang('rp_clear'), array('class' => 'btn btn-small btn-primary'));
        }
    ...
    <table class="table table-striped user-table">
		<thead>
			<tr>
				...
				<th class='id'><?php echo lang('rp_id'); ?></th>
				<th><?php echo lang('rp_username'); ?></th>
				<th><?php echo lang('rp_display_name'); ?></th>
				<th><?php echo lang('rp_email'); ?></th>
		...
		<tfoot>
			<tr>
                <td colspan="<?php echo $numColumns; ?>">
					<?php
					echo lang('rp_with_selected');

					if ($filter_type == 'deleted') :
					?>
					<input type="submit" name="restore" class="btn" value="<?php echo lang('rp_action_restore'); ?>" />
					<input type="submit" name="purge" class="btn btn-danger" value="<?php echo lang('rp_action_purge'); ?>" onclick="return confirm('<?php e(js_escape(lang('us_purge_del_confirm'))); ?>')" />
					<?php else : ?>
					<input type="submit" name="activate" class="btn" value="<?php echo lang('rp_action_activate'); ?>" />
					<input type="submit" name="deactivate" class="btn" value="<?php echo lang('rp_action_deactivate'); ?>" />
					<input type="submit" name="ban" class="btn" value="<?php echo lang('rp_action_ban'); ?>" />
					<input type="submit" name="delete" class="btn btn-danger" id="delete-me" value="<?php echo lang('rp_action_delete'); ?>" onclick="return confirm('<?php e(js_escape(lang('us_delete_account_confirm'))); ?>')" />
					<?php endif;?>
				</td>
			</tr>
		</tfoot>
