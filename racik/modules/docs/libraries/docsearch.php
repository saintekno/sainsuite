 ...
 * Class docSearch
 *
 * Implements basic search capabilities for racik docs. Includes application,
 * core racik, and module docs.
 */
class DocSearch
{
        ...
        foreach ($map as $dir => $file) {
            ...
            if (preg_match_all(
                ...
                foreach ($matches as $match) {
                    ...
                    // Does $folder contain RPPATH?
                    if (strpos($folder, RPPATH) !== false) {
                        // Does $folder contain RPPATH . 'docs'?
                        if (strpos($folder, RPPATH . 'docs') !== false) {
                            $result_url = str_replace(RPPATH . 'docs', '', $folder);
                            $result_url = '/docs/developer' . $result_url . '/' .  $filename;
                        } elseif (strpos($folder, RPPATH . 'modules/') !== false) {
                            // Does $folder contain RPPATH . 'modules/'?
                            ...
                            if (substr($folder, -strlen('/docs/developer')) == '/docs/developer') {
                                ...
                                $result_url  = str_replace(RPPATH . 'modules/', '/docs/developer/', $result_url);
                                $result_url .= '/' . $filename;
                            } else {
                                $result_url  = str_replace(RPPATH . 'modules/', '/docs/application/', $folder);
