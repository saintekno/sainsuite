...
/**
 * The Docs controller provides access to Racik's documentation and allows the
 * developer to provide access to documentation for their application.
 *
 * @package Racik\Modules\Docs\Controllers\Docs
 */
class Docs extends Base_Controller
{
    ...
    protected $docsTypeRp  = 'developer';
    ...
    public function __construct()
    {
        ...
        } elseif ($this->docsGroup == $this->docsTypeRp
            && ! $this->showDevDocs
    ...
    public function search()
    {
        ...
        if ($terms) {
            ...
            if ($this->showDevDocs) {
                $search_folders[] = RPPATH . $this->docsDir;
            }
            ...
            foreach (Modules::list_modules() as $module) {
                ...
                if (strpos($path, "{$module}/docs") !== false) {
                    $path = str_replace(realpath(RPPATH), rtrim(RPPATH, '/'), $path);
                    ...
                    // Add if showDevDocs and if $path contains RPPATH.
                    if ($this->showDevDocs && strpos($path, RPPATH) !== false) {
                        $search_folders[] = $path;
                    }
                }
            }
    ...
    private function buildSidebar()
    {
        ...
        if ($this->docsGroup == $this->docsTypeApp) {
            ...
            $data['docs'] = $this->getFolderFiles(RPPATH . $this->docsDir, $this->docsTypeRp);
        }
     ...
     * ('application', 'racik', or the name of the module).
    ...
    private function getFolderFiles($folder, $type, $ignoredFolders = array())
    {
        ...
        // If these docs are located in the /application/docs or /racik/docs
    ...
    private function getModuleDocs()
    {
        ...
        foreach (Modules::list_modules() as $module) {
            ...
            if ($this->docsGroup == $this->docsTypeRp) {
                $path .= '/' . $this->docsTypeRp;
            } else {
                // For Application docs, ignore the 'developers' folder.
                $ignored_folders[] = $this->docsTypeRp;
            }
    ...
    private function readPage($segments = array())
    {
        ...
        if (count($segments)) {
            ...
            if ($type != $this->docsTypeMod
                ...
                $type = $this->docsTypeRp;
            }
        }

        // First try to load from Activities or racik.
        switch ($type) {
            case $this->docsTypeRp:
                $content = is_file(RPPATH . $this->docsDir . '/' . $file) ? file_get_contents(RPPATH . $this->docsDir . '/' . $file) : '';
                break;
        ...
        if (empty($content)) {
            ...
            $addPath = $type == $this->docsTypeRp ? '/' . $this->docsTypeRp . '/' : '/';
    ...
    private function postProcess($content)
    {
        ...
        foreach ($xml->xpath('//a') as $link) {
            ...
            // This includes 'racik/' if it was missed during the conversion.
            if (substr($href, 0, strlen('racik/')) == 'racik/') {
                $href = substr($href, strlen('racik/'));
            }
