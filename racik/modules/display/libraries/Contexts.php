...
class Contexts
{
    ...
    protected static function init()
    {
        ...
        log_message('debug', 'Display/Contexts library loaded');
    }
    ...
    public static function setContexts($contexts = array(), $siteArea = null)
    {
        ...
        log_message('debug', 'Display/Contexts setContexts has been called.');
    }
    ...
    public static function render_menu($mode = 'text', $order_by = 'normal', $top_level_only = false, $benchmark = false)
    {
        ...
        if (empty($contexts) || ! is_array($contexts)) {
            die(self::$ci->lang->line('rp_no_contexts'));
        }
        ...
        foreach ($contexts as $context) {
            ...
            if (self::$ci->auth->has_permission($viewPermission)
                ...
                $title    = self::$ci->lang->line("rp_context_{$context}");
    ...
    public static function context_nav($context = null, $class = 'dropdown-menu', $ignore_ul = false)
    {
        ...
        foreach (self::$actions as $module => $config) {
            ...
            // view it.
            if (self::$ci->auth->has_permission('Racik.' . ucfirst($module) . '.View')
    ...
    {
        if (empty($name)) {
            self::$errors = lang('display_no_context_name');
            return false;
        }
        ...
        if (! in_array($lowerName, $contexts)) {
            ...
            if (! write_config('application', array('contexts' => $contexts), null)) {
                self::$errors[] = lang('display_cant_write_config');
                return false;
            }
        }
        ...
        $temp = addLanguageLine('application_lang.php', array("rp_context_{$lowerName}" => $name), 'english');
        if (! $temp) {
