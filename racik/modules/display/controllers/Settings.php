 ...
 *
 * Manages the keyboard shortcuts used in the Racik admin interface.
 * 
 */
class Settings extends Admin_Controller
{
    ...
    public function __construct()
    {
        ...
        $this->auth->restrict('Racik.Display.View');
        $this->auth->restrict('Racik.Display.Manage');
    ...
    public function index()
    {
        ...
        Template::set('current', config_item('display.current_shortcuts'));
        Template::set('settings', $this->settings_lib->find_all_by('module', 'core.display'));
    ...
    private function addShortcut()
    {
        ...
        // Read available shortcuts from the application config.
        $availableActions = config_item('display.current_shortcuts');
    ...
    private function removeShortcut()
    {
        ...
        $availableActions = $this->settings_lib->find_all_by('module', 'core.display');
        if (array_key_exists($action, $availableActions)) {
            return $this->settings_lib->delete($action, 'core.display');
        }
    ...
    private function saveSettings($settings = array())
    {
        ...
        foreach ($settings as $action => $shortcut) {
            $updatedSetting = $this->settings_lib->set($action, $shortcut, 'core.display');
            if (! $updatedSetting) {
                $updated = false;
            }
        }
